...
const CustomerListComponenet = () => {
  ...
  useEffect(() => {
    const fetchDriverList = async () => {
      setIsLoading(true);
      try {
        const response = await getMethod(endPoints.Admin.getAllCustomerList);
        if (response?.data?.statusCode === 200) {
          setCustomerList(response?.data?.data as CustomerType[]);
          setIsLoading(false);
        } else {
          setIsLoading(false);
...
export default CustomerListComponenet;
